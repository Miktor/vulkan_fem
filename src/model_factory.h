...
namespace vulkan_fem
{
	...
	public:
		...
		//	return std::shared_ptr<Model<3>>(new Model<3>(std::make_shared<Element2dTo3d>(std::make_shared<TriangleElement>()),
		//									  vertices,
		//									  indices,
		//									  0.2e12, 0.3)); // 200GPa, 0.3 Young, Poisson's for steel
		//}
	...
	private:
		...
		static void generateCylinder(const precision r,
									 const precision h,
									 std::vector<Vertex3> &vertices,
									 std::vector<uint16_t> &indices)
		{
			...
			generateEndCapVertexData(SIDES, r, half_h, index, elIndex,
									 vertices, indices);
			generateEndCapVertexData(SIDES, r, -half_h, index, elIndex,
									 vertices, indices);
		}

		static void generateEndCapVertexData(const uint32_t sides,
											 const precision r,
											 const precision y,
											 uint16_t& index,
											 uint16_t& elIndex,
