...
class ScalarElement : public Element<DIM> {
 public:
  ...
  virtual std::vector<precision> CalcShape(
      const std::vector<precision>& ip) const = 0;

  // calculate partial derivatives
  virtual MatrixFixedRows<DIM, precision> CalcDShape(
      const std::vector<precision>& ip) = 0;
...
class TetrahedronElement : public ScalarElement<3> {
 public:
  ...
  virtual std::vector<precision> CalcShape(
      const std::vector<precision>& ip) const override {
...
class Element2dTo3d : public ScalarElement<3> {
 ...
 public:
  ...
  virtual MatrixFixedRows<3, precision> CalcDShape(
    ...
    MatrixFixedRows<3, precision> converted =
        MatrixFixedRows<3, precision>::Zero(3, dshape.cols());
