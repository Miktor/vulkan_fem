...
class Model {
 public:
  ...
  Model(std::shared_ptr<ScalarElement<DIM>> element_type, const std::vector<Vertex3>& vertices,
        const std::vector<uint16_t>& indices, const std::vector<Constraint>& constraints,
  ...
  ElementMatrix BuildGlobalStiffnesMatrix() {
    ...
    for (uint32_t index = 0; index < element_inidices_.size();) {
      ...
      for (uint16_t sub_index = 0; sub_index < element_count && index + sub_index < element_inidices_.size();
           ++sub_index) {
 ...
 private:
  ...
  // [ 0  Ni
  // [ Ni Ni
  MatrixFixedRows<DIM> calcElementMatrix(std::shared_ptr<ScalarElement<DIM>> element_type,
  ...
  // [ 0   Niy
  // [ Niy Nix
  Eigen::Matrix<precision, 3, 6> makeStrainMatrix(const uint16_t element_count,
                                                  const MatrixFixedRows<DIM>& elem_matrix) {
